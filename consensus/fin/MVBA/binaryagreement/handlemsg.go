...
func (ba *BA) handle_loopmsg(loopinfo *LoopInfo, oldbuf chan pb.BAMsg, newbuf chan pb.BAMsg) {
	...
	for {
		...
		if msg.MVBARound != (loopinfo.mvbaround) {
			//get an old round msg
			panic("get a msg from unexpected MVBARound")
		}
		if msg.BARound != (loopinfo.baround) {
			//get an old round msg
		...
		if msg.Type == 4 {
			...
			ba.finishCH <- msg
			continue
		} else if msg.Loop < (loopinfo.loop) {
			continue
		} else if msg.Loop > (loopinfo.loop) {
			newbuf <- msg
			continue
