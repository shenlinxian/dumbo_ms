...
func NewBA(num int, id int, mvbaround int, baround int, input bool, secondinput chan bool, outputCH chan bool, msgin chan pb.BAMsg, msgout chan pb.BAMsg) *BA {
...
func (ba *BA) sendval(loopinfo *LoopInfo) {
	...
	for {
		...
		valmsg := pb.BAMsg{
			ID:        (ba.id),
			MVBARound: (loopinfo.mvbaround),
			BARound:   (loopinfo.baround),
			Loop:      (loopinfo.loop),
			Type:      1,
			Value:     val,
		}
