...
import (
	...
	rbc "dumbo_ms/consensus/fin/MVBA/rbcwithfinish"
	pb "dumbo_ms/structure"
	"encoding/binary"
)

func New_mvba(node_num int, K int, id int, round int, rbcMsgOutCH chan pb.RBCMsg, baMsgOutCH chan pb.BAMsg, rbcmsgCH []chan pb.RBCMsg, bamshCH chan pb.BAMsg, input []byte, check_input_rbc func([]byte, []bool, chan bool) bool, outputCH chan []byte, broadcastheights []bool) *MVBA {
	...
	mvba := &MVBA{
		...
		rbcMsgOutCH: rbcMsgOutCH,
		baMsgOutCH:  baMsgOutCH,
...
func (mvba *MVBA) Launch() {
	...
	for {
		...
		newba := ba.NewBA(mvba.Node_num, mvba.ID, mvba.Roundmvba, mvba.Roundba, input, sencondinput, output, bamsgCH, mvba.baMsgOutCH)
		go newba.Launch()
		select {
		case <-mvba.MVBA_done:
			return
...
func (mvba *MVBA) start_rbc_leader(input []byte) {
	leader := rbc.NewBroadcast_leader(mvba.ID, mvba.ID, mvba.Roundmvba, mvba.Node_num, input, mvba.rbcout1, mvba.rbcout2, mvba.rbcmsgCHs[mvba.ID-1], mvba.rbcMsgOutCH, mvba.MVBA_done)
...
func (mvba *MVBA) start_rbc_follower(leader int) {
	follower := rbc.NewBroadcast_follower(mvba.ID, leader, mvba.Roundmvba, mvba.Node_num, mvba.rbcout1, mvba.rbcout2, mvba.rbcmsgCHs[leader-1], mvba.rbcMsgOutCH, mvba.check_input_rbc, mvba.broadcastheights, mvba.MVBA_done, mvba.closeval)
