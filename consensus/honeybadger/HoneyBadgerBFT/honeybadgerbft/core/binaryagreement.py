...
def binaryagreement(sid, pid, N, f, coin, input, decide, broadcast, receive):
    ...
    #filename="/home/luy/golang_projects/src/dumbo_ms/log/data%d.json" % (pid)
    #with open(filename, 'a', encoding='utf-8') as file:
    #    file.write('inside consensus binaryagreement' + "\n")
    ...
    while True:  # Unbounded number of rounds
        ...
        #filename="/home/luy/golang_projects/src/dumbo_ms/log/data%d.json" % (pid)
        #with open(filename, 'a', encoding='utf-8') as file:
        #    file.write('inside consensus binaryagreement multicast aux' + "\n")
        broadcast(('AUX', r, w))
...
def set_new_estimate(*, values, s, already_decided, decide,pid):
    if len(values) == 1:
        v = next(iter(values))
        if v == s:
